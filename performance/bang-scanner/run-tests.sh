...
if ( ls "$TMPDIR"/bang-scan-* 2>&1 > /dev/null )
then
	echo "Error: directory $TMPDIR not empty."
...
do
	...
	echo python3 "$BANG_SCANNER" -c "$basedir/bang.config" -f "$FW"
	{ time (python3 "$BANG_SCANNER" -c "$basedir/bang.config" -f "$FW" ) ; } 2>> "$PERFTESTFILE"
	echo "done! Recording timings..."
	echo -n '"duration": ' >> "$PERFTESTFILE"
	python3 -c "import sys,pickle; print(pickle.load(open(sys.argv[1],'rb'))['session']['duration'])" "$TMPDIR"/bang-scan-*/bang.pickle >> "$PERFTESTFILE"
	echo "Cleaning up..."
	echo rm -rf "$TMPDIR"/bang-scan-*
	rm -rf "$TMPDIR"/bang-scan-*
	((i++))
	if [[ $i != $ITERATIONS ]]
	then
		echo "}, "  >> "$PERFTESTFILE"
